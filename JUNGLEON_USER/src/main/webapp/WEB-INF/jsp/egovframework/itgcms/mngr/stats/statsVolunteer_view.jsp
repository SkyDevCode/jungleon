...
<%@ taglib prefix="ufn" uri="/WEB-INF/tlds/egovfn.tld"%>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="box box-primary">
		                <div class="box-header with-border">
		                  	<h3 class="box-title">
		                  		...
		                  		<c:set var="titleStr" value="${ufn:deCode(searchVO.schOption, 'DAY_M,월(간) 일자별,MONTH,년(간) 월별,YEAR,년(간) 전체,STATUS, 상태별,SEX, 성별', '')}"/>
		                  		${searchVO.schDateFrom} ${ufn:IIF(searchVO.schDateFrom eq searchVO.schDateEnd,'',endDateStr)}${titleStr} 사회봉사 통계
							</h3>
		                </div><!-- /.box-header -->
						<div class="box-body">
							...
							<div class="row margin-bottom">
								<div class="col-sm-12 form-inline" id="searchBox">
									...
									<select name="schOption" id="schOption" class="form-control">
											...
											<option value="SEX" ${ufn:selected(searchVO.schOption, 'SEX') }>성별</option> 
									</select>
					...
					<div class="box">
						<div class="box-header with-border">
							<span><i class="fa fa-circle-o text-light-orange"></i> 기간 내 방문자 총 <i id="cnt"></i> 명  <c:if test="${searchVO.schOption ne 'STATUS' and searchVO.schOption ne 'SEX' }">(※상태 승인 기준 방문자 통계 입니다.)</c:if></span>
							...
							<div class="row">
								<div class="col-sm-12">
								    <table id="example1" class="table table-bordered">
								        ...
								        <thead>
											<tr>
												<th scope="col">${ufn:deCode(searchVO.schOption, 'STATUS,상태,SEX,성별', '기간')}</th>
												<th scope="col">이용수</th>
											</tr>
										</thead>
								        <tbody>
												...
												<tr>
													<td class="tac">
														<c:choose>
															...
															<c:when test="${searchVO.schOption eq 'SEX' }">
																${result.cSex }
															</c:when>	
														</c:choose>
													</td>
...
//<![CDATA[
	...
	function getExcel(){
		...
		f.schDateFrom.value=f.start.value;
		f.schDateEnd.value=f.end.value;
		f.action = "/excel/mngrStatsVolunteerExcelDown.do";
		f.method = "post";
	    f.submit();
    }
//]]>
