...
<%
/**
 * @파일명 : user_qna_view.jsp
 * @파일정보 : 1대1질문답변형게시판 조회 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
...
<c:if test="${not empty boardMap.bdAnswer }">
	<dl class="infor">
		<dt class="t">답변 내용</dt>
		<dd class="writer">
	<c:choose>
		<c:when test="${boardMap.bdHtmlyn eq 'Y'}">
			<c:out value="${ufn:decodeXss(boardMap.bdAnswer)}" escapeXml="false" />
		</c:when>
		<c:otherwise>
			<c:out value="${ufn:stripXss(boardMap.bdAnswer) }" escapeXml="false" />
		</c:otherwise>
	</c:choose>
		</dd>
	</dl>
	<dl class="infor">
		<dt class="t">답변 작성자</dt>
		<dd class="writer"><c:out value="${boardMap.bdAnswerwriter }" /></dd>
	</dl>
</c:if>
