		           ...
		           <c:choose>
					   ...
					   <c:when test="${result.uploadType eq 'mv' }">
						...
						<c:when test="${result.uploadType eq 'stream' }">
							<iframe width="100%"     title="유튜브 영상 연결"  height="100%" src="https://www.youtube.com/embed/${result.streamUrl}" frameborder="1" allowfullscreen></iframe></a>
						</c:when>
					</c:choose>
...
<c:if test="${not isMobile}">
	...
	<div class="modalpopup" id="pop_${result.popupIdx}">
		...
		<div class="cont"><c:out value="${result.edit_contents }" escapeXml="false"></c:out></div>
			...
			<c:when test="${result.uploadType eq 'stream' }">
			<iframe width="100%" height="100%"  title="유튜브 영상 연결" src="https://www.youtube.com/embed/${result.streamUrl}" frameborder="1" allowfullscreen></iframe></a>
			</c:when>
		</c:choose>
		</div>
		<c:if test = "${result.popupFile != ''}">
