...
<%
/**
 * @파일명 : _mngr__webzine_list.jsp
 * @파일정보 : 관리자 갤러리형(목록) 게시판 목록 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
                    ...
                    <li>
								<div class="w">
									<div class="thumb">
										...
										<a href="#none" style="float:right;" onclick="fn_goView('<c:out value="${result.bdIdx }" />')">
										<img src="<c:url value="/comm/viewImage.do?f=${ufn:getDownloadLink('','gallery',file,'thumb') }" />" alt="<c:out value="${ufn:quot(alt) }" />"/></a>
									</div>
									<div class="cont">
<c:if test="${boardconfigVO.bcGroupyn eq  'Y' }">
										<p class="subt"><c:out value="${result.bdCodeName }" /></p>
</c:if>
										<p class="subj"><a href="#none" onclick="fn_goView('<c:out value="${result.bdIdx }" />')"><c:out value="${result.bdTitle }" /></a></p>
										<p class="txt"><c:out value="${ufn:cutString(ufn:getAntiHtml(result.bdContent, '1'), 100, '...') }" /></p>
										<p class="date"><c:out value="${fn:substring( result.regdt, 0, 10) }" /></p>
									</div>
								</div>
							</li>
</c:forEach>
