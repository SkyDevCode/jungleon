									...
									<table id="example1" class="table table-bordered">
										...
										<c:forEach var="result" items="${resultList}" varStatus="status">
											<tr>
												...
												<td class="tb_left bg_id"><a href="#none" onclick="fn_goView('<c:out value="${result.id }" />'); return false;"><c:out value="${result.comNm}" /></a></td>
												<%-- <td class="tb_left bg_id tac"><a href="javascript:fn_showAccessPass('<c:out value="${result.id}" />');"><c:out value="${result.comNm}" /></a></td> --%>
												<td class="bg_name tac"><c:out value="${result.ceoNm}" /></td>
												<td class="bg_nickname tac"><c:out value="${result.addr01}" /> <c:out value="${result.addr02s}" /></td>
												<td class="tac"><c:out value="${result.officeTel01 }" />-<c:out value="${result.officeTel02 }" />-<c:out value="${result.officeTel03 }" /></td>
												<td class="bg_status tac"><c:out value="${ufn:deCode(result.comTp, 'V006000001,개인기업,V006000002,법인기업','-') }" /></td>
											</tr>
										</c:forEach>
...
<div class="modal" id="typeForAccess">
 ...
 <script type="text/javascript">
	...
	function fn_goRegist() {
		location.href = "company_input.do";
	}
	...
	function fn_chkDel() {
		if ($("input[name='chkId']:checked").size() == 0) {
			...
			if (confirm("선택한 회원을 삭제하시겠습니까?")) {
				document.listForm.action = "company_delete_chkProc.do";
	...
	function fn_goView(id) {
		document.acForm.action = "company_edit.do?" + ItgJs.fn_replaceQueryString(queryString, "id", id);
