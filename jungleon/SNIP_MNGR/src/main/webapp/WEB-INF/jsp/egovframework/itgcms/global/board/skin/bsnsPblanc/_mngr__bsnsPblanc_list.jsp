...
function fn_getExcel(siteCode, bcid) {
	if (confirm("해당 게시판의 전체 목록을 엑셀로 저장하시겠습니까?")) {
		...
		document.listForm.action = "/exceldownload/module/${menuCode}_bsnsPblanc_bsnsPblanc.do";
							...
							<tbody>
								<c:forEach var="result" items="${noticeList}" varStatus="status">
									<tr>
										...
										<c:if test="${boardconfigVO.bcGroupyn eq  'Y' }">
											...
											<td class="tac"><c:out value='${result.recStat }'/></td>
								...
								<c:forEach var="result" items="${resultList}" varStatus="status">
									...
									<tr>
										...
										<%-- <td class="bg_regdt tac"><c:out value='${result.recSdt } ~ ${result.recEdt }' /></td> --%>
