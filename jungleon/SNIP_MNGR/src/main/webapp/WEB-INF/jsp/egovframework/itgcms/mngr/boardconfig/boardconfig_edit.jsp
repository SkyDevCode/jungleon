...
<c:if test="${searchVO.act eq 'UPDATE' and boardCnt > 0}">
						...
						<table class="table table-bordered tp2">
							...
							<tbody>
								<tr>
									...
									<td><input type="text" name="bcName" id="bcName" maxlength="100" value="<c:out value="${result.bcName}"/>" class="required form-control input-sm f-wd-200" title="게시판 이름" /></td>
								</tr>
								<tr>
...
<!-- 												</td> -->
								...
								<tr class="_SKINOPTION_ forGALLERY forWEBZINE forMOVIE forCUSTOM forEPSEARCH forBPNEWS" style="display: none;">
								...
								<tr class="_SKINOPTION_ forDEFAULT forQNA forSTARTUP" style="display: none;">
								...
								<tr class="_SKINOPTION_ forCUSTOM customColumnLowTr" style="display: none; background-color:white;">
									<td colspan="4" id="customColumnLowTd">
										...
										<div class="customColumnLowDiv custom">
											<!-- <span><label>코드</label> : <input type="text" name="columnCode" value="" class="required form-control input-sm f-wd-100" title="컬럼코드" readonly="readonly"/></span> -->
											<input type="hidden" name="columnCode" value=""/>
											<span><label>컬럼명</label> : <input type="text" name="columnName" maxlength="30" value="" class="required form-control input-sm f-wd-200" title="컬럼명" /></span>
											<span><label>컬럼타입</label> : <cct:codeTag pcode="columnType" tagType="select" tagName="columnType" addOptData="data-type,etc1" tagTitle="컬럼타입" className="form-control input-sm f-wd-150 typeSelect required columnType" useNullOpt="N"/></span>
 ...
 var setting = {
	...
	function fn_addCode(){
		...
		$.ajax({
			...
			data:"schCode=<c:out value="${result.bcId}" />&cpcode="+cpcode+"&cname="+cname,
			type:"post",
			dataType:"json",
			success : function(data){
				if(data.result == "0"){
	...
	function fn_updateProc(treeId, treeNode, newName) {
		...
		$.ajax({
			...
			, type : "post"
			, data : "act=UPDATE&schCode="+treeNode.id+"&cname="+newName
			, dataType : "json"
			, success : function(data){
				if(data.result == 0){
