...
function fn_getExcel(siteCode, bcid) {
	if (confirm("해당 게시판의 전체 목록을 엑셀로 저장하시겠습니까?")) {
		if (bcid == "maPuNoti") {
			document.listForm.action = "/exceldownload/module/${menuCode}_maPuNoti_${bcid}.do";
		} else {
			...
			<form name="listForm" id="listForm" method="post" onsubmit="fn_search(); return false;">
				...
				<div class="row margin-bottom">
					<div class="col-sm-8 form-inline">
						...
						<c:choose>
							<c:when test="${bcid eq 'maPuNoti'}"></c:when>
							<c:otherwise>
							...
							<tbody>
								...
								<c:forEach var="result" items="${resultList}" varStatus="status">
									...
									<tr>
										...
										<td class="bg_title" style="padding-left: ${lpadding}px;"><a href="#none" onclick="fn_goView('<c:out value="${result.bdIdx }" />')">
										<c:choose>
