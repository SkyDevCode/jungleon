				...
				<table id="example1" class="table table-bordered table-striped sati mainform">
					...
					<tbody>
						<c:forEach var="result" items="${resultList }">
							...
							<c:choose>
								<c:when test="${empty result.answer1 }">
								<td>응답 없음</td>
								</c:when>
								<c:otherwise>
								<td><a href="#none" class="btn btn-xs btn-primary" onclick="fn_viewSatisfaction('<c:out value="${result.menuCode }" />'); return false;">의견보기</a></td>
								</c:otherwise>
							</c:choose>
							</tr>
						</c:forEach>
