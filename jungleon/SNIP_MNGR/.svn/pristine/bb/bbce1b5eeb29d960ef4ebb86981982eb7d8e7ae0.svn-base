...
<%
/**
 * @파일명 : _mngr__qna_list.jsp
 * @파일정보 : 관리자 1대1질문답변형게시판 목록 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
...
<c:if test="${boardconfigVO.bcGroupyn eq  'Y' }">
					...
					<div class="col-sm-4 text-right">
						<button type="button" onclick="fn_getExcel('${mngrSessionVO.currSiteCode}', '${bcid }');" class="btn btn-default btn-sm"><i class="fa fa-file-excel-o"></i>&nbsp;&nbsp;엑셀다운로드</button>
...
<c:if test="${boardconfigVO.bcGroupyn eq  'Y' }">
									<td><c:out value="${result.bdCodeName }" /></td>
</c:if>
