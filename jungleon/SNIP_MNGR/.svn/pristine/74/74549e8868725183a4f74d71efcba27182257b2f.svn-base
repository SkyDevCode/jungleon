...
<%
/**
 * @파일명 : _mngr__spBussiness_view.jsp
 * @파일정보 : 관리자 사업공고 게시판 조회 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
 * @hsk1218 2020. 03. 18. 최초생성
 * @---------------------------------------
 * @author (주)아이티굿 개발팀
 * @since 2009. 01.14
...
$(document).ready(function(){
	...
	ItgJs.fn_datePickerRange("#recSdt", "#recEdt");
						...
						<div class="row">
							<div class="col-xs-12 table-responsive">
								<table class="table table-bordered tp2">
									...
									<tbody>
										<tr>
											<th class="t"><label for="meDescription">메뉴설명</label></th>
											<td colspan="2" class="">
												<textarea name="meDescription" id="meDescription" style="width:100%; height:100px;" class="" title="메뉴설명"><c:out value="${boardMap.meDescription}" /></textarea>
											</td>
										</tr>
										<c:if test="${boardconfigVO.bcFileyn eq 'Y' }" >
											<tr>
										...
										<tr>
											<th class="t"><label for="bsTarget">사업대상</label></th>
											<td colspan="2" class="">
												<textarea name="bsTarget" id="bsTarget" style="width:100%; height:100px;" class="" title="사업대상"><c:out value="${boardMap.bsTarget }" /></textarea>
											</td>
										</tr>
										<tr>
											<th class="t"><label for="bsContent">사업내용</label></th>
											<td colspan="2" class="">
												<textarea name="bsContent" id="bsContent" style="width:100%; height:100px;" class="" title="사업내용"><c:out value="${boardMap.bsContent }" /></textarea>
											</td>
										</tr>
										<tr>
											<th class="t"><label for="ofLocation">위치</label></th>
											<td colspan="2" class="">
												<textarea name="ofLocation" id="ofLocation" style="width:100%; height:100px;" class="" title="위치"><c:out value="${boardMap.ofLocation }" /></textarea>
											</td>
										</tr>
										<tr>
											<th class="t"><label for="cgInfo">담당자</label></th>
											<td colspan="2" class="">
												<textarea name="cgInfo" id="cgInfo" style="width:100%; height:100px;" class="" title="담당자"><c:out value="${boardMap.cgInfo }" /></textarea>
											</td>
										</tr>
										<tr>
											<th class="t"><label for="bdContent">상세 지원 내용</label></th>
												...
												<c:choose>
													<c:when test="${boardMap.bdHtmlyn eq 'Y'}">
														<c:if test="${boardconfigVO.bcEditoryn ne 'Y'}">
															<textarea name="bdContent" id="bdContent" style="width:100%; height:300px;" title="상세 지원 내용"></textarea>
														...
														<c:if test="${boardconfigVO.bcEditoryn eq 'Y'}">
															<textarea name="bdContent" id="bdContent" style="width:100%; height:300px;" title="상세 지원 내용"><c:out value="${ufn:decodeXss(boardMap.bdContent)}" escapeXml="false" /></textarea>
														</c:if>
													</c:when>
													<c:otherwise>
															...
															<textarea name="bdContent" id="bdContent" style="width:100%; height:300px;"  title="상세 지원 내용"><c:out value="${boardMap.bdContent}" escapeXml="false" /></textarea>
													</c:otherwise>
												</c:choose>
												<c:if test="${boardconfigVO.bcEditoryn eq 'Y'}">
				                                    <!-- 에디터를 화면에 출력합니다. -->
				                                    <script type="text/javascript">
					                                    ...
					                                    myeditor.inputForm = 'bdContent';           // textarea의 ibdContentd 이름입니다. 주의: name 속성 이름이 아닙니다.
										...
										<tr>
											<th class="t"><label for="bsPuLiUrl">사업공고 확인 링크 URL(현재창)</label></th>
											<td colspan="3">
												<input type="text" name="bsPuLiUrl" id="bsPuLiUrl" class="form-control input-sm f-wd-600" value="<c:out value="${boardMap.bsPuLiUrl }"/>" maxlength="50" title="사업공고 확인 링크 URL(현재창)" />
												<span><h4>※ 입력 예 : https://www.snip.or.kr</h4></span>
											</td>
										</tr>

										<tr>
											<th class="t"><label for="hpGlLiUrl">홈페이지 바로가기 링크 URL(새창)</label></th>
											<td colspan="3">
												<input type="text" name="hpGlLiUrl" id="hpGlLiUrl" class="form-control input-sm f-wd-600" value="<c:out value="${boardMap.hpGlLiUrl }"/>" maxlength="50" title="홈페이지 바로가기 링크 URL(새창)" />
												<span><h4>※ 입력 예 : https://www.snip.or.kr</h4></span>
											</td>
										</tr>
