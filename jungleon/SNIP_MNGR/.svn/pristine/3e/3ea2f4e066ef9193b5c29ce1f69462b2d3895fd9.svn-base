		...
		<div class="box">
			<div class="box-body">
				<div class="col-md-12" style="padding:0;">
					<form name="totalSearchConfig" id="totalSearchConfig" method="post" onsubmit="fn_egov_save(); return false;" class="form-horizontal">
						<table class="table table-bordered table tp2">
							...
							<tr>
								<th style="vertical-align: middle;"><span>통합검색메뉴코드</span></th>
								<td>
									<label>${siteconfigVO.totalSearchMenuCode}</label>
								</td>
							</tr>
							<tr>
								<th style="vertical-align: middle;"><span>사용구분</span></th>
								<td>
									<select name="totalSearch" id="totalSearch" class="form-control input-sm f-wd-200" title="통합검색">
						        		...
						        		<option value="onlyMe" ${ufn:selected(siteconfigVO.totalSearch, 'onlyMe') }>${mngrSessionVO.currSiteName}만 검색</option>
						        	</select>
								</td>
							</tr>
						</table>
...
function fn_egov_save() {
	frm = document.totalSearchConfig;
	if(Validator.validate(frm)){
		$.ajax({
			url : '${urlInfo.menuCode}_edit_mngrTotalSearch_proc.ajax',
			data : $("#totalSearchConfig").serialize(),
			type : "POST",
