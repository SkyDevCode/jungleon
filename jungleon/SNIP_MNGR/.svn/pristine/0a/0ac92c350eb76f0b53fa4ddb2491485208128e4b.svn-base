...
<%
/**
 * @파일명 : _mngr__default_view.jsp
 * @파일정보 : 관리자 일반 게시판 조회 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
				...
				<form name="bbsForm" id="bbsForm" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="bdContentText"/>
						<div class="row">
									...
									<tbody>
										...
										<tr>
											...
											<c:choose>
												...
												<c:otherwise>
												<input type="text" name="bdTitle" id="bdTitle" class="required form-control input-sm f-wd-600" value="<c:out value="${boardMap.bdTitle }" escapeXml="false"/>" maxlength="50" title="제목" />
												</c:otherwise>
											</c:choose>
											</td>
										</tr>
										<c:if test="${boardconfigVO.bcGroupyn eq 'Y' }">
											<tr>
												<th class="t"><label for="bdCode">분류</label></th>
												<td colspan="3" class="last">
													<ora:selectCodeList selectName="bdCode" selectedValue="${boardMap.bdCode }" codeList="${codeList }" selectTitle="분류"/>
												</td>
											</tr>
										</c:if>
										<%-- <tr>
											<th class="t"><span>담당자</span></th>
											<td colspan="3"><c:out value="${boardMap.charger }" /></td>
										</tr> --%>
										<tr>
							...
							<%-- <tr>
								<th class="t"><span>SNS 공유</span></th>
								<td>
									<label><input type="checkbox" id="naverPostingCheck" name="postingExternalSite" value="naver" <c:out value="${naver.smAccessible == 'Y' ? 'checked' : ''}"/>/>&nbsp;&nbsp;네이버</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<label><input type="checkbox" id="facebookPostingCheck" name="postingExternalSite" value="facebook" <c:out value="${facebook.smAccessible == 'Y' ? 'checked' : ''}"/>/>&nbsp;&nbsp;페이스복</label>&nbsp;&nbsp;&nbsp;&nbsp;
								</td>
							</tr> --%>
										<c:if test="${searchVO.act eq 'REGIST' }">
										<%-- <tr>
											<th class="t">외부사이트 연동 정보</th>
											<td>
												...
												<c:if test="${twitter != null}">
													<label><input type="checkbox" id="twitterPostingCheck" name="postingExternalSite" value="twitter" <c:out value="${twitter.smAccessible == 'Y' ? 'checked' : ''}"/>/>&nbsp;&nbsp;트위터</label>&nbsp;&nbsp;&nbsp;&nbsp;
												</c:if>
												<c:if test="${instargram != null}">
													<label><input type="checkbox" id="instargramPostingCheck" name="postingExternalSite" value="instargram" <c:out value="${instargram.smAccessible == 'Y' ? 'checked' : ''}"/>/>&nbsp;&nbsp;인스타그램</label>&nbsp;&nbsp;&nbsp;&nbsp;
												</c:if>
											</td>
										</tr> --%>
										</c:if>
