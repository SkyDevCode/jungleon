...
<%
/**
 * @파일명 : _mngr__qna_view.jsp
 * @파일정보 : 관리자 1대1질문답변형게시판 조회 스킨
 * @수정이력
 * @수정자 수정일 수정내용
 * @------- ------------ ----------------
...
<c:if test="${ufn:getProhibitRegEx() ne ''}">
	...
	<c:if test="${(boardMap.regmemid eq searchVO.schRegmemid) or (searchVO.act eq 'REGIST')}">
...
<c:choose>
	<c:when test="${boardconfigVO.bcEditoryn == 'Y'}">
		<c:if test="${(boardMap.regmemid eq searchVO.schRegmemid) or (searchVO.act eq 'REGIST')}">
			...
			//웹접근성검사
			if(!fn_checkAccessability("문의 내용","bdContent", myeditor)) return false;
			frm.noHTML.value = myeditor.getBodyText();
		</c:if>
			//답변 에디터 내용검사
			if(myeditor2.inputLength() == 0 && $("#bdAnswer").val()==""){
		        var data = myeditor2.getImages();
		        if (data == null) {
		            alert("답변 내용을 입력하세요.");
		            myeditor2.editAreaFocus();
		            return false;
		        }
		    } else {
				frm.noHTML2.value = myeditor2.getBodyText();
			<c:if test="${ufn:getProhibitRegEx() ne ''}">
				if (prohibitRegEx.test(frm.noHTML2.value)) {
					alert("답변 내용에 금지어를 포함하고 있습니다.");
					return false;
				}
			</c:if>
			}
			...
			//웹접근성검사
			if(!fn_checkAccessability("답변 내용","bdAnswer", myeditor2)) return false;
			frm.noHTML2.value = myeditor2.getBodyText();
	</c:when>
	<c:otherwise>
			if($("#bdContent").val()=="") {
		...
		<c:if test="${ufn:getProhibitRegEx() ne ''}">
			...
			if(!fn_checkAccessability("문의 내용","bdContent")) return false;
		...
		<c:if test="${ufn:getProhibitRegEx() ne ''}">
				if (prohibitRegEx.test(frm.bdAnswer.value)) {
					alert("답변 내용에 금지어를 포함하고 있습니다.");
					$("#bdAnswer").focus();
					return false;
				}
			...
			if(!fn_checkAccessability("답변 내용","bdAnswer")) return false;
	</c:otherwise>
</c:choose>
...
<div class="row">
	<div class="col-xs-12">
		<div class="box">
			<div class="box-body">
				...
				<form name="bbsForm" id="bbsForm" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="noHTML2" value="" />
		...
		<c:if test="${boardconfigVO.bcGroupyn eq 'Y' }">
										...
										<tr>
											...
											<td colspan="3">
													<c:if test="${searchVO.act eq 'UPDATE' }">
														<c:if test="${boardMap.regmemid ne searchVO.schRegmemid}">
												  			<input type="text" class="form-control f-wd-600 required" name="bdTitle" value="<c:out value="${boardMap.bdTitle }" />" maxlength="50" title="제목" readonly/>
												  		</c:if>
												  		<c:if test="${boardMap.regmemid eq searchVO.schRegmemid}">
												  			<input type="text" class="form-control f-wd-600 required" name="bdTitle" value="<c:out value="${boardMap.bdTitle }" />" maxlength="50" title="제목" />
												  		</c:if>
													</c:if>
													<c:if test="${searchVO.act ne 'UPDATE' }">
														<input type="text" name="bdTitle" class="form-control f-wd-600 required" maxlength="50" title="제목" />
													</c:if>
											</td>
										</tr>
										<tr>
										...
										<tr>
											<th class="t"><label for="bdContent">문의 내용</label></th>
											<td colspan="3">
							<c:if test="${searchVO.act eq 'UPDATE' }">
								...
								<c:if test="${boardMap.regmemid ne searchVO.schRegmemid}">
									<c:choose>
										<c:when test="${boardMap.bdHtmlyn eq 'Y'}">
											<c:set var="bdContent" value="${ufn:decodeXss(boardMap.bdContent)}"/>
										</c:when>
										<c:otherwise>
											<c:set var="bdContent" value="${ufn:stripXss(boardMap.bdContent)}"/>
										</c:otherwise>
									</c:choose>
											  	<input type="hidden" id="bdContent" name="bdContent" value="<c:out value="${bdContent}"/>">
												<c:out value="${bdContent}" escapeXml="false"/>
								</c:if>
								<c:if test="${boardMap.regmemid eq searchVO.schRegmemid}">
									<c:choose>
										<c:when test="${boardMap.bdHtmlyn eq 'Y'}">
											<c:if test="${boardconfigVO.bcEditoryn ne 'Y'}">
												<textarea name="bdContent" id="bdContent" style="width:100%; height:300px;" class="required" title="문의 내용"></textarea>
											...
											<c:if test="${boardconfigVO.bcEditoryn eq 'Y'}">
												<textarea name="bdContent" id="bdContent" style="width:100%; height:300px;" class="required" title="문의 내용"><c:out value="${ufn:decodeXss(boardMap.bdContent)}" escapeXml="false" /></textarea>
											</c:if>
										</c:when>
										<c:otherwise>
							...
							<c:if test="${searchVO.act ne 'UPDATE' }">
												<textarea name="bdContent" id="bdContent" class="required" style="width:100%; height:100px;" title="문의 내용"></textarea>
								<c:if test="${boardconfigVO.bcEditoryn eq 'Y'}">
											          <!-- 에디터를 화면에 출력합니다. -->
											          <script type="text/javascript">
											            var myeditor = new cheditor();              // 에디터 개체를 생성합니다.
										...
										<tr>
											<th class="t"><label for="bdAnswer">답변 내용</label></th>
											<td colspan="3">
												<textarea name="bdAnswer" id="bdAnswer" style="width:100%; height:300px;" class="required" title="답변 내용"><c:out value="${boardMap.bdAnswer }" /></textarea>
<c:if test="${boardconfigVO.bcEditoryn eq 'Y'}">
				                                    <!-- 에디터를 화면에 출력합니다. -->
				                                    <script type="text/javascript">
					                                    ...
					                                    myeditor2.config.editorBgColor  = '#ffffff'; 	// 에디터 기본배경색입니다.
					                                    myeditor2.inputForm = 'bdAnswer';           // textarea의 id 이름입니다. 주의: name 속성 이름이 아닙니다.
					                                    myeditor2.run();                             // 에디터를 실행합니다.
					                                    $("#bdAnswer").removeClass("required");
				                                    </script>
</c:if>
											</td>
										</tr>
										<tr>
											<th class="t"><label for="bdAnswerwriter">답변작성자</label></th>
											<td colspan="3">
												<input type="hidden" name="bdAnswerwriter" value="<c:out value="${boardMap.bdAnswerwriter }"/>" title="답변작성자" />
												<c:out value="${boardMap.bdAnswerwriter }" />
											</td>
										</tr>
		<c:if test="${boardconfigVO.bcFileyn eq 'Y' }" >
										<tr>
