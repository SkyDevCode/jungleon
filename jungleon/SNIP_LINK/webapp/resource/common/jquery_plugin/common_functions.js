...
function ItgJs(){
	...
	ItgJs.fn_chkEmail = function(fldEmail, strErrorMessage){
		var strExp = /^[_A-Za-z0-9-]+(\.[_A-Za-z0-9-]+)*@[_A-Za-z0-9-]+(\.[A-Za-z0-9]+)*(\.[A-Za-z]{2,})$/;
...
console.log(sum)
	...
	ItgJs.fn_checkPass = function(pass1,  pass2){
		...
		if(pass1.length<10){	return "비밀번호는 영문, 숫자, 특수문자의 조합으로 10자리 이상 입력해주세요.";}
		if(!pass1.match(/[a-zA-Z0-9]*[^a-zA-Z0-9\n]+[a-zA-Z0-9]*$/ )){return "비밀번호는 영문, 숫자, 특수문자의 조합으로 10자리 이상 입력해주세요.";}
	...
	*/
	...
	ItgJs.selectBoxAllLocalNumber = function(id, value) {
		var localNumberArr = [
			"02", "051", "053", "032", "062", "042", "052", "031", "033", "043", "041", "063", "061", "054", "055", "064", "070", "010", "011", "016", "017", "018", "019"
			];
		$("#" + id).append("<option value=''>선택</option>");
		ItgJs.selectBoxForeach(localNumberArr, id, value);
	}
	...
	// 도로명 주소만 입력받을 필드 우편번호, 주소, 상세주소  설정
	// ItgJs.getDaumAddressStreet({zip:'우편번호필드명',addr1:'주소필드명',addr2:'상세주소필드명'});
	ItgJs.getDaumAddressStreet = function(config) {
		new daum.Postcode({
			oncomplete: function(data) {
				$("#btnCloseLayer").focus();
                ...
                if(data.bname !== '' && /[동|로|가]$/g.test(data.bname)){
                    extraRoadAddr += data.bname; // 상세주소에 동명을 추가할 경우 주석 해제.
                }
                // 건물명이 있고, 공동주택일 경우 추가한다.
                // 아파트 외에 건물명 까지 추가 할경우 data.apartment 주석 해제.
                if(data.buildingName !== '' && data.apartment === 'Y'){
                   extraRoadAddr += (extraRoadAddr !== '' ? ', ' + data.buildingName : data.buildingName);
                }
                // 표시할 참고항목이 있을 경우, 괄호까지 추가한 최종 문자열을 만든다.
                if(extraRoadAddr !== ''){
                    extraRoadAddr = ' (' + extraRoadAddr + ')'; //건물, 아파트명 추가시 괄호 붙일경우 주석 해제
                }
...
//                document.getElementById("sample4_jibunAddress").value = data.jibunAddress;
                ...
                $("#" + config.zip).val(data.zonecode);
                $("#" + config.addr1).val(roadAddr);

                // 참고항목 문자열이 있을 경우 해당 필드에 넣는다.
                if(roadAddr !== ''){
                	$("#" + config.addr2).val(extraRoadAddr);
                } else {
                	$("#" + config.addr2).val('');
                }
	...
	ItgJs.fn_mainSearch = function(menuCode,frm){
		...
		location.href="../contents/"+menuCode+".do?schStr="+encodeURIComponent(frm.schStr.value)+"&siteCode=SNIP";
...
/*		if(isOldIe){
	...
	(function($) {
	    $.ajaxSetup({
	    	//beforeSend: ItgJs.showLoading, //화면 깜빡임때문에 제거
	        ...
	        //,complete: ItgJs.hideLoading //화면 깜빡임때문에 제거
	    });
	})(jQuery);
