$(document).on('ready', function() {
	...
	$(".sub-tab__smenu").each(function(){
		...
		//console.log(totalWidth, win_w);
		var sub_tab_idx = $(this).index();
		var sub_tab_5dep = $(this).find(".depth5Menu_list>div").length;
		$(this).addClass("tab_5dep_"+sub_tab_5dep);
		if (totalWidth < win_w){
			$(this).addClass("tab_5dep_"+sub_tab_5dep);
			console.log("aa");
			}else{
				
				var slideNo = $(this).find(".s_active").index();
				$(this).find(".depth5Menu_list").addClass("sub-tab__5dep_list_rolling"+sub_tab_idx);
				dep5Menu_slider('.sub-tab__5dep_list_rolling'+sub_tab_idx,true);
				$('.sub-tab__5dep_list_rolling').slick('slickGoTo', slideNo);
