$(document).on('ready', function() {
    ...
    //$(".depth5Menu_list span").attr("tabindex","0");

    $(".sub-tab__menu .active>a").attr("title","선택됨");
    $(".dep5_menu.s_active>a").attr("title","선택됨");
    ...
    $(".sub-tab__4dep").each(function() {
        ...
        if (page_tab == false) {
            ...
            if (totalWidth < win_w) {
                ...
                $(this).find(".sub-tab__4dep_list").before('<ul class="slick_arrow_wrap"><li class="slick-prev"><button>이전목록으로</button></li><li class="slick-next"><button>다음목록으로</button></li></ul>')
    ...
    function advisory_person_slide(item) {
        ...
        $(item).on('afterChange', function(event, slick, currentSlide, nextSlide){
        	...
        	$(item).closest('.advisory_person_list').find('select option').prop('selected', false);
        	$(item).closest('.advisory_person_list').find('select option').eq(currentSlide).prop('selected', true);
        });
    }
    ...
    $('.menu__set-sns .menu__set-sns_close').on('click', function() {
        $('.menu__sns-group').hide();
    });
    ...
    $(document).on("click", ".focus-tab .menu", function(e) {
        e.preventDefault();
    ...
    $(".focus-tab .menu").on("click", function(e) {
    ...
    var $contents = $('.content__area *');
    var zoom = 1;
    //zoom in
    $(".btn_font_plus").on("click", function() {
        zoom = zoom + 0.01;
        $contents.css('zoom', zoom);
    });

    //zoom out
    $(".btn_font_minus").on("click", function() {
        zoom = zoom - 0.01;
        $contents.css('zoom', zoom);
    });
});
