			...
			<div class="box-body">
				...
				<div class="row">
					<div class="col-xs-12 table-responsive">
		                <h4 class="box-title" style="width:70%;">기본설정</h4>
			            <button type="button" class="btn btn-primary" onclick="javascript:fn_mainPageMake();" style="float: right;margin-top: -40px;cursor: pointer;">사용자 메인페이지 생성</button>
...
<%-- 										            <div id="faviUrl" class="form-group" style="margin-bottom:0px">
				                                	...
				                                	<c:if test="${not empty siteconfigVO.siteLogo }">
				                                    	...
				                                    	<p style="display: block;max-width: 300px;margin-top: 10px;"><img style="max-width: 100%;" src="<c:url value="/comm/viewImage.do?f=${ufn:getDownloadLink('','system',siteconfigVO.siteLogo,siteconfigVO.siteLogo) }" />" alt="<c:out value="${siteconfigVO.siteLogo}" />" /></p>
				                                  	</c:if> --%>
									</td>
								</tr>
								<tr>
...
<%-- 								<tr>
						...
						<table class="table table-bordered tp2">
							...
							<tbody>
								<tr>
									<th class="t"><label for="recentBd">게시판최근글영역1</label></th>
									<td id="recentBd"></td>
								</tr>
								<tr>
									<th class="t"><label for="recentBd2">게시판최근글영역2</label></th>
									<td id="recentBd2"></td>
								</tr>
								<tr>
									<th class="t"><label for="recentBdJoin">게시판JOIN최근글영역</label></th>
									<td id="recentBdJoin"></td>
								</tr>
								<tr>
									<th class="t"><label for="recentPrm">프로그램최근글영역</label></th>
									<td id="recentPrm"></td>
								</tr>
							</tbody>
						</table>
...
<%--       <%@ include file="../../global/module/socialmedia/comConnectScript_inc.jsp" %> --%>
...
function fn_mainPageRecentAjax(tempCode, cfgparam) {
	//console.log("tempCode : "+tempCode);
	$.ajax({
		url : "/_mngr_/mainrecent_comm_check.ajax",
		type : "post" ,
		async : false,
		data : {
			siteCode : '${mngrSiteVO.siteCode}',
			templCode : tempCode,
			cfgparam : cfgparam+"Cnt"
		}, success : function(result){
			if(result.recentCount == 0){
				$("#"+cfgparam).parent().remove();
			}else{
				var mainPageboard = $("#"+cfgparam).empty();
				var html = "";
				for (var x =0; x < result.recentCount; x++){
					html += "<select class=\"form-control\" style=\"display:inline-block; width : 200px;\" name=\""+cfgparam+"\" id=\"board_"+x+"\">";
					for (var y=0; y < result.menuDesc.length;y++) {
						var isSelected = false;
						if (result.selectedMenu != null) {
							var selectedMenu;
							selectedMenu = result.selectedMenu[x].split("/");
							if(selectedMenu[0] == result.menuDesc[y].menuCode){
								isSelected = true;
							}
						}
						html += "<option value=\""+result.menuDesc[y].menuCode+"\" "+(isSelected?"selected='selected'":"")+">"+result.menuDesc[y].menuName+"</option>";
					}
					var selectedCode = "";
					var selectedCount = "";
					if (result.selectedMenu != null) {
						var selectedCodes = result.selectedMenu[x].split("/");
						selectedCount = selectedCodes[1];
						if(selectedCodes[2] != null){
							selectedCode = selectedCodes[2];
						}
					}
					html += "</select>&nbsp;&nbsp;<input name=\""+cfgparam+"Limit\" type=\"number\" value=\""+selectedCount+"\" style=\"width:40px; display:inline-block;\"/>&nbsp;개<br/>";
					html += "<input name=\""+cfgparam+"Code\" type=\"text\" value=\""+selectedCode+"\" style=\"width:100px; display:inline-block;\"/>&nbsp;<br/>";
				}
				mainPageboard.html($(html));
			}
		}
	});
}

function fn_mainPageRecentAjax2(tempCode, cfgparam) {
	//실제 메인페이지 최신글관련 프로그래밍이 되어있는 목록,신착자료,사서추천도서,책 읽어주는 도서관,국외DB.
	var useMainProgram = "N10302000000,N20500000000,N20600000000,N10401000000";
	$.ajax({
		url : "/_mngr_/mainrecent_comm_check.ajax",
		type : "post" ,
		async : false,
		data : {
			...
			if(result.recentCount == 0){
				...
				for (var x =0; x < result.recentCount; x++){
					html += "<select class=\"form-control\" style=\"display:inline-block; width : 200px;\" name=\""+cfgparam+"\" id=\"board_"+x+"\">";
					for (var y=0; y < result.menuDesc.length;y++) {
						var menuCode = result.menuDesc[y].menuCode;
						if(useMainProgram.indexOf(menuCode) != -1){
							var isSelected = false;
							if (result.selectedMenu != null) {
								var selectedMenu;
								selectedMenu = result.selectedMenu[x].split("/");
								if(selectedMenu[0] == menuCode){
									isSelected = true;
								}
							}
							html += "<option value=\""+menuCode+"\" "+(isSelected?"selected='selected'":"")+">"+result.menuDesc[y].menuName+"</option>";
						}
					}
					var selectedCount = "";
					if (result.selectedMenu != null) {
...
<c:if test="${siteconfigVO.tempCode ne '' and siteconfigVO.tempCode ne null}">
fn_mainPageRecentAjax('${siteconfigVO.tempCode}','recentBd');
fn_mainPageRecentAjax('${siteconfigVO.tempCode}','recentBd2');
fn_mainPageRecentAjax('${siteconfigVO.tempCode}','recentBdJoin');
fn_mainPageRecentAjax2('${siteconfigVO.tempCode}','recentPrm');
</c:if>
//라디오버튼 값 변경 체크
...
$('input[type="radio"]').on('ifChecked', function(event){
	...
	if(opt == "siteLogogubun"){
		...
		fn_mainPageRecentAjax(tempCode,'recentBd');
		fn_mainPageRecentAjax(tempCode,'recentBd2');
		fn_mainPageRecentAjax(tempCode,'recentBdJoin');
		fn_mainPageRecentAjax2(tempCode,'recentPrm');
	}else{
