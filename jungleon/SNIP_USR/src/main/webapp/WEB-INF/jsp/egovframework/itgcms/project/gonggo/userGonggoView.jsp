<%@page import="egovframework.itgcms.common.DefaultVO"%>
...
<%-- <%
Object name= request.getAttribute("searchVO");
DefaultVO searchVO = (DefaultVO)request.getAttribute("searchVO");
String tt=searchVO.getSchId();
if(request.isSecure()) {
%>
<script type="text/javascript">
location.href="http://www.snip.or.kr/SNIP/contents/Business1.do?schMemid=snip&schM=view&page=1&viewCount=10&schId=<%=tt%>";
</script>
<%
}
%> --%>
                        ...
                        <div class="view-info">
									<c:choose>
										<c:when test="${result.stat eq 'ing' }">
											<span class="icon-state ing">진행</span>
										</c:when>
										<c:when test="${result.stat eq 'end' }">
											<span class="icon-state end">마감</span>
										</c:when>
									</c:choose>
					            <strong class="title">
...
<div class="btn-wrap btn-appli">
					...
					<a href="#this" onclick="if(confirm('기업회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false; self.close(); " class="btn btn-blueline2 icon-book btn-block">세무회계 온라인제출</a>
                               		<%
                               		} else{
                               		%>
					...
					<a href="#this" onclick="if(confirm('기업회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;" class="btn btn-blueline2 icon-book btn-block">세무회계 온라인제출</a>
                                        </c:if>
                                    </c:when>
                                    <c:when test="${result.reqtarget == 20}">
                                       <c:if test="${not empty userSessionVO}">
                                       		 <%if(CustomUtil.checkMemberType(MemberType.Company)) {
	                               			%>
	                               		<a href="#this" onclick="if(confirm('개인회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;self.close(); " class="btn btn-blueline icon-check btn-block">신청하기</a>
						<a href="#this" onclick="if(confirm('개인회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;self.close(); " class="btn btn-blueline2 icon-book btn-block" >세무회계 온라인제출</a>
	                               		<%
	                               		} else{
	                               		%>
								...
								<a href="https://www.snip.or.kr/SNIP/contents/Guidance3.do" target="_blank" class="btn btn-blueline2 icon-book btn-block"  title="새창연결">세무회계 온라인제출</a>
                                      ...
                                      <c:if test="${empty userSessionVO}">
                                      		<a href="#this" onclick="if(confirm('개인회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;" class="btn btn-blueline icon-check btn-block">신청하기</a>
						<a href="#this" onclick="if(confirm('개인회원으로 로그인 후 이용바랍니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;" class="btn btn-blueline2 icon-book btn-block"  title="새창연결">세무회계 온라인제출</a>
                                    ...
                                    <c:when test="${result.reqtarget == 30}">
                                        <c:if test="${sessionScope.memType eq 'N'}">
	                                         <c:if test="${empty userSessionVO}">
	                                      		<a href="#this" onclick="onclick="if(confirm('로그인이 필요한 서비스입니다. 로그인페이지로 이동하시겠습니까')){location.href='https://www.snip.or.kr/SNIP/contents/snipLogin.do'} return false;" class="btn btn-blueline icon-check btn-block">신청하기</a>
...
<c:if test="${not empty userSessionVO}">
		<iframe id="hidden" name="hiddenifr" title="내용 없음" src="https://www.snip.or.kr/iframe.jsp">
</iframe>
</c:if>
<script>
...
function fn_goRegist2() {
	...
	function fn_goList(){
		...
		location.href = "https://www.snip.or.kr/SNIP/contents/Business1.do?" + query;
	}
