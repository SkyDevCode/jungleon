                ...
                <!-- 게시판리스트 -->
                    ...
                    <div class="board_top">
                        <form name="listForm" method="post" onsubmit="fn_search(); return false;">
                            <fieldset>
                                ...
                                <!--     <select name="schBdcode" id="schBdcode" title="구분" class="select_box">
										<option value="">구분</option>
										<option value="_spBussiness01">스타트업</option>
										<option value="_spBussiness02">기술 사업화</option>
										<option value="_spBussiness03">펀드투자</option>
										<option value="_spBussiness04">스케일업</option>
										<option value="_spBussiness05">협력 생태계 조성</option>
										<option value="_spBussiness06">기술 개발</option>
									</select> -->
									<label for="schBdcode" class="blind">검색구분</label>
	                				<ora:selectCodeList selectName="schOpt4" selectedValue="s${searchVO.schOpt4 }" codeList="${codeList }" selectTitle="구분" className="select_box"/>
                    ...
                    <div class="board-list_wrap">
                        <table class="board-list">
                             <caption>
						            <p class="ir-text">번호,사업명,제목,접수기간,첨부,담당자,작성일,조회수로 구성된 <c:out value="${boardconfigVO.bcName }" /> 게시판 리스트</p>
						        </caption>
                          ...
                          <tbody>
                          	<c:forEach var="result" items="${resultList}" varStatus="status">
								...
								<tr>
									...
									<td class="num">${ufn:deCode(result.etcname,'57,스타트업,55,기술 사업화,56,기술 개발,58,스케일업,59,협력 생태계 조성','펀드투자')}</td>
				    				<td class="subject">
...
//<[[!CDATA[
	...
	function fn_goView(id) {
		...
		<c:if test="${searchVO.schOpt5 eq 'fdaegi'}">
		location.href="https://www.snip.or.kr/SNIP/contents/Business23.do?" + tmpQuery;
		</c:if>
		<c:if test="${searchVO.schOpt5 eq 'end'}">
		location.href="https://www.snip.or.kr/SNIP/contents/Business22.do?" + tmpQuery;
		</c:if>
		<c:if test="${searchVO.schOpt5 eq 'ing'}">
		location.href="https://www.snip.or.kr/SNIP/contents/Business21.do?" + tmpQuery;
		</c:if>
	}
	...
	function fn_search(){
		...
		tmpQuery = ItgJs.fn_replaceQueryString(tmpQuery, "schOpt4", f.schOpt4.value);
