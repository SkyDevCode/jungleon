...
<c:choose>
	...
	<c:when test="${searchVO.act eq 'UPDATE' }">
											...
											<tr>
												...
												<span id="pwHelp" class="help">영문, 숫자, 특수문자 조합으로 9자 이상 </span>
												</div>
												</td>
...
<c:if test="${searchVO.act eq 'REGIST' }">
												...
												<td>
													<select name="groupCode" id="groupCode" title="부서" class="form-control input-sm f-wd-200">
														...
														<c:forEach var="group" items="${groupList }">
															<option value="<c:out value="${group.orCode }" />" ${ufn:selected(result.groupCode, group.orCode )}><c:out value="${group.orName }" /></option>
														</c:forEach>
													</select>
												</td>
											...
											<tr>
												<th class="t"><span>특별업무권한</span></th>
												<td colspan="3">
													<cct:codeTag pidx="10742" tagType="checkbox" tagId="mngRole" tagName="mngRole" tagTitle="특별업무권한" selectedValue="${result.mngRole }" className="form-control input-sm f-wd-150"/>
												</td>
											</tr>
											<tr>
...
function fn_egov_save() {
	...
	if(Validator.validate(frm)){
		...
		var message = Validator["validate-max-length"](frm.mngWork.value,"담당업무",'60');
