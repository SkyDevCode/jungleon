...
<div class="board-list_wrap">
    <table class="board-list">
        <caption>
        	<c:choose>
	        	<c:when test="${menuVO.menuCode == 'Business33'}">
	        		<p class="ir-text">번호,센터명,사업명,제목,담당자,작성일,조회수로 구성된 <c:out value="${boardconfigVO.bcName }" /> 게시판 리스트</p>
	        	</c:when>
	        	<c:otherwise>
	        		<p class="ir-text">번호,센터명,사업명,제목,접수기간,첨부,담당자,작성일,조회수로 구성된 <c:out value="${boardconfigVO.bcName }" /> 게시판 리스트</p>
	        	</c:otherwise>
        	</c:choose>
        </caption>
            <colgroup class="n_mobile">
				...
				<c:if test="${boardconfigVO.bcFileyn eq  'Y' }"><c:set var="colspan" value="${colspan + 1 }" />
                	<col class="col5" style="width:6%">
				</c:if>
                <col class="col6" style="width:7%">
                <col class="col7" style="width:7%">
                <col class="col8" style="width:7%">
            </colgroup>
        <thead>
         <tr>
             	<th scope="col">번호</th>
				...
				<tr>
    				...
    				<td class="subject">
        				<a href="#none" onclick="fn_goView('<c:out value="${result.bdIdx}" />')">
       					<%-- <c:out value="${result.bdTitle }" /> --%>
       					<c:out value="${ufn:cutString(result.bdTitle, 50, '...')}" />
        				</a>
