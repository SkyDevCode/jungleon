...
<%-- E: 추가 영역 --%>
                <div class="board-wrap">
                    <div class="board-view">
                        <div class="view-info">                            
                            <strong class="title">
                                <c:out value="${result.title }" /> <span class="label-statu <c:out value="${ufn:IIF(result.statusCd eq '1103', 'end', '') }" />"><c:out value="${result.statusName }" /></span>
                            </strong>
                            <div class="other-info">
                                <span>등록일 : <c:out value="${result.regDt }" /></span>
                            </div>
                        </div><!--// view-info-->
                        <div class="view-content">
                            <c:out value="${fn:replace(result.question, '\\\n' , '<br />') }" escapeXml="false"/>
						</div><!--// view-content-->
                            ...
                            <ul class="prev-next-wrap">
                                <li class="prev">
									...
									<c:if test="${not empty prev }">
										<a href="#none" onclick="fn_goView('<c:out value="${prev.seq }" />'); return false;"><c:out value="${prev.title }" /></a>
									</c:if>
                                </li>
                                <li class="next">
                                    <strong>다음글</strong>
									...
									<c:if test="${not empty next }">
										<a href="#none" onclick="fn_goView('<c:out value="${next.seq }" />'); return false;"><c:out value="${next.title }" /></a>
									</c:if>
                                </li>
                            </ul><!--// prev-next-wrap-->
						</div><!--// board-bottom-->      
