								...
								<form name="form1" id="form1" method="post" onsubmit="fn_save(); return false;">
									...
									<table class="table table-bordered tp2">
										...
										<tbody>
											<tr>
												<th class="t">메뉴권한명</th>
												<td><input type="text" name="menuAuthName" id="menuAuthName" title="메뉴권한명" class="required form-control input-sm " maxlength="20" /></td>
											</tr>
											<tr>
												<th class="t">관리사이트</th>
												<td>
													...
													<c:if test="${fn:length(siteList ) eq 1}">
													${siteList[0].siteName}
													<input type="hidden" name="menuAuthSitecode" id="menuAuthSitecode" value="${siteList[0].siteCode}"/>
													</c:if>
												</td>
											</tr>
										</tbody>
...
//<![CDATA[
	...
	$(document).ready(function(){
		fn_setMenuAuthSitecode('${schSiteCode}');
	})
	...
	function fn_save(){
		...
		if(Validator.validate(frm)){
			...
			if(confirm("등록 하시겠습니까?")){
				frm.action="/_mngr_/menuauth/mnauthgroup_input_proc.do";
				frm.submit();
			}
		}
	}
	...
	function fn_setMenuAuthSitecode(siteCode){
...
/* 			check: {
	...
	function initTree(){
		zNodes = [];
		$.ajax({
			url:"/commons/comm_treemenu_list.ajax",
