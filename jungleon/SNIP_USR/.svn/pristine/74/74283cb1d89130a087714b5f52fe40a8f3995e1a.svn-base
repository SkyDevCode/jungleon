...
<c:when test="${searchVO.act eq 'REGIST' }">
	...
	<c:set var="parentIndex" value="${result.cidx}" />
	<c:set var="parentCode" value="${result.ccode}" />
	<c:set var="parentFullcode" value="${result.cpfullcode}>${result.ccode}" />
	<c:if test="${empty result}">
		<c:set var="parentCode" value="0" />
		<c:set var="parentFullcode" value="0" />
	</c:if>
	<c:set var="parentName" value="${result.cname}" />
	<c:set var="cidx" value="" />
...
<c:when test="${searchVO.act eq 'UPDATE' }">
	...
	<c:set var="parentIndex" value="${result.cpidx}" />
	<c:set var="parentCode" value="${result.cpcode}" />
	<c:set var="parentFullcode" value="${result.cpfullcode}" />
	<c:set var="parentName" value="${result.pcodename}" />
	<c:set var="cidx" value="${result.cidx}" />
...
function fn_printHelp(result){
	...
	if(result == 0){
		codeCheck = true;
		$("#ccodeHelp").css("color","green");
		$("#ccodeHelp").text("사용가능한 코드 입니다.");
	}else if(result == 1) {
		$("#ccodeHelp").css("color","red");
		$("#ccodeHelp").text("중복된 코드 입니다.");
	}else if(result == 2) {
		$("#ccodeHelp").css("color","red");
		$("#ccodeHelp").text("코드명은 영문자로 시작해야 합니다.");
	}else if(result == 3) {
		$("#ccodeHelp").css("color","red");
		$("#ccodeHelp").text("코드명은 영문(대소문자), 숫자, -(하이픈) 만 입력 할 수 있습니다.");
	}else{
		$("#ccodeHelp").css("color","");
...
<form name="form1" id="form1" method="post" onsubmit="fn_save(); return false;">
	...
	<input type="hidden" name="cidx" value="<c:out value="${cidx}" />" />
	<input type="hidden" name="cpidx" value="<c:out value="${parentIndex}" />" />
	<input type="hidden" name="cpcode" value="<c:out value="${parentCode}" />" />
	<input type="hidden" name="cpfullcode" value="<c:out value="${parentFullcode}" />" />
	<div class="row">
		<div class="col-xs-12 table-responsive">
			<table class="table table-bordered tp2">
				...
				<tbody>
					<tr>
						<th class="t"><span>상위코드 [INDEX]</span></th>
						<td><c:out value="${parentFullcode} [${parentIndex}]" /></td>
					</tr>
					<tr>
					...
					<tr>
						<th class="t"><label for="em">코드${ufn:IIF(searchVO.act eq 'UPDATE',' [INDEX]','')}</label></th>
						<td>
							<c:choose>
				        		<c:when test="${searchVO.act eq 'REGIST' }">
				        		...
				        		<c:otherwise>
				        			<c:out value="${ccode} [${cidx}]" />
				        		</c:otherwise>
				        	</c:choose>
				            <span id="ccodeHelp" class="help-block" style="margin-bottom:0;"><c:if test="${searchVO.act eq 'REGIST' }">등록 후에 수정 할 수 없습니다.</c:if></span>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th class="t"><label for="etc1">기타옵션1</label></th>
						<td>
							<input type="text" name="etc1" id="etc1" maxlength="300" class="form-control input-sm f-wd-400" title="비고1" value="<c:out value="${etc1}" />" />
						</td>
					</tr>
					<tr>
						<th class="t"><label for="etc2">기타옵션2</label></th>
						<td>
							<input type="text" name="etc2" id="etc2" maxlength="300" class="form-control input-sm f-wd-400" title="비고2" value="<c:out value="${etc2}" />" />
						</td>
					</tr>
					<tr>
...
<c:if test="${not (fn:startsWith(ccode,'_') or ccode eq 'board')}">
