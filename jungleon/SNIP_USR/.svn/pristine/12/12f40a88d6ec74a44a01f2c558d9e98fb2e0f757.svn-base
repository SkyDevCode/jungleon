...
<div class="board-wrap">
                    <div class="board-view">
                       ...
                       <div class="view-content">
							...
							<c:out value="${ufn:decodeXss(result.contents)}" escapeXml="false" />
						</div><!--// view-content-->
                                ...
                                <c:choose>
                                    <c:when test="${result.reqtarget == 10}">
                                       <c:if test="${not empty userSessionVO}">
                                        	<a href="#" class="btn btn-blueline icon-check btn-block" title="새창 열림">신청하기</a>
											<a href="/SNIP/contents/Guidance3.do" target="_blank" class="btn btn-blueline icon-book btn-block">세무회계 온라인제출</a>
                                        </c:if>
                                      <c:if test="${empty userSessionVO}">
                                        	<a href="#this" onclick="alert('기업회원으로 로그인 후 이용바랍니다'); return false;" class="btn btn-blueline icon-check btn-block">신청하기</a>
											<a href="#this" onclick="alert('기업회원으로 로그인 후 이용바랍니다'); return false;" class="btn btn-blueline icon-book btn-block">세무회계 온라인제출</a>
                                        </c:if>
                                    </c:when>
                                    <c:when test="${result.reqtarget == 20}">
                                       <c:if test="${not empty userSessionVO}">
                                      		<a href="javascript:fn_goRegist2();" class="btn btn-blueline icon-check btn-block">신청하기</a>
											<a href="/SNIP/contents/Guidance3.do" target="_blank" class="btn btn-blueline icon-book btn-block">세무회계 온라인제출</a>
                                        </c:if>
                                      <c:if test="${empty userSessionVO}">
                                      		<a href="#this" onclick="alert('개인회원으로 로그인 후 이용바랍니다'); return false;" class="btn btn-blueline icon-check btn-block">신청하기</a>
											<a href="#this" onclick="alert('개인회원으로 로그인 후 이용바랍니다'); return false;" class="btn btn-blueline icon-book btn-block">세무회계 온라인제출</a>
                                        </c:if>
                                    </c:when>
...
function fn_goRegist2() {
	...
	function fn_goList(){
		...
		query = ItgJs.fn_replaceQueryString(query, "schOpt3", "${searchVO.schOpt3 }");
		query = ItgJs.fn_replaceQueryString(query, "schM", "list");
		location.href = "https://www.snip.or.kr/SNIP/contents/Business31.do??" + query;
	}
